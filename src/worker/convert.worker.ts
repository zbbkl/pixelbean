...
import { aiModelById, runAiMask, type AiProgress } from './aiMaskClient';
...
async function handle(request: ConvertRequest): Promise<void> {
  ...
  try {
    ...
    // 可选 AI 抠图：**服务端推理**（把缩放后的像素 POST 给自家服务，拿回掩码），
    // 再走与确定性路径相同的管线与可靠度门。
    // 任何失败（离线 / 服务未部署 / 限流 / 超时）都**不抛给用户**，而是落回确定性路径，
    ...
    if (request.options.aiBackground === true) {
      try {
        aiMask = await runAiMask(
          source,
          aiModelById(request.options.aiModel),
          (progress: AiProgress) =>
            post({ seq: request.seq, kind: 'progress', stage: progress.stage }),
          // 资源基址由主线程给出（支持子路径部署）；缺省退回站点根（worker 位于 /assets/）
          request.assetBase ?? new URL('../', self.location.href).href
        );
      } catch (error) {
        aiFailed = true;
        aiMask = null;
